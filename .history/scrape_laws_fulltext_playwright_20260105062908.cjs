...
// Entry point: scrape a chunk of laws for parallel jobs
// Usage: node scrape_laws_fulltext_playwright.cjs [chunkIndex] [totalChunks]
const allLaws = JSON.parse(fs.readFileSync('all_laws.json', 'utf-8'));
...
const chunk = allLaws.slice(chunkIndex * chunkSize, (chunkIndex + 1) * chunkSize);
// for (const law of chunk) { ... scrape ... }
// ...existing code...
// fs.writeFileSync(`scraped_laws_chunk_${chunkIndex}.json`, JSON.stringify(results, null, 2));
// Playwright script to extract full text and metadata for each law
const fs = require('fs')
const path = require('path')
...
const outputPath = path.join(__dirname, 'public', 'all_laws_full.json')
