...
const batchPath = path.join(
  __dirname,
  'public',
  `missing_laws_batch_${batchIdx}.json`
)
const outputPath = path.join(
  __dirname,
  'public',
  `all_laws_scraped_missing_batch_${batchIdx}.json`
)
const errorPath = path.join(
  __dirname,
  `scrape_errors_log_batch_${batchIdx}.json`
)
...
async function main() {
  ...
  for (let i = 0; i < batch.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Batch ${batchIdx}: scrape-нати дотук: ${scrapedLaws.length}, грешки: ${errors.length}`
    )
  }
}
