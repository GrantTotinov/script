...
async function main() {
  ...
  for (let i = 0; i < missingLaws.length; i += BATCH_SIZE) {
    ...
    console.log(`Scraping batch ${i/BATCH_SIZE+1} (${batch.length} закона)...`);
    const results = await Promise.all(batch.map(async (law) => {
    ...
    console.log(`Дотук scrape-нати: ${scrapedLaws.length}, грешки: ${errors.length}`);
  }
